...
class Ur5():
    ...
    def step(self,action):
        ...
        reward, terminal = self.get_reward(position,rpy)

        return state, reward, terminal

    def reset(self):

        ######getting the current joint position
        self.current_joints = INIT
        self.client.send_goal(self.initial)
        self.client.wait_for_result()
        self.target_generate()

        ###get_pos returns two array values 'position' and 'rpy'
        ##### Out of them only data for 'position' is considered
        position, rpy = self.get_pos(link_name='ee_link')

        return self.get_state(rpy,position)
        #return np.array(position)

    def get_state(self,rpy,position):
        #x, y, z of goal
        ...
        state = np.concatenate((pose_6,orient_6),axis=None)
